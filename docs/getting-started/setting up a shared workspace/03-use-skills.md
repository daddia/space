---
title: Use skills
description: Install @daddia/skills in any repo and invoke delivery activity skills from Cursor, Claude Code, and other Markdown-aware IDEs.
---
...
- How to install `@daddia/skills` and keep it up to date
...
1. Installs `@daddia/skills` and runs `sync-skills`, which copies the skill library into a `skills/` directory at your workspace root.
...
> If you scaffolded your workspace with `@daddia/create-space`, the package and postinstall script are already configured. Run `pnpm install` to sync the skills.
...
> The source of truth is the npm package. Do not edit files inside `skills/` that came from `@daddia/skills` -- your changes will be overwritten on the next update. For project-local skills, add them alongside the synced ones and they will be preserved.
