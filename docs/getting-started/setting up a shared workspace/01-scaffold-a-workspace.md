...
description: Create a standards-compliant, agent-ready delivery workspace in minutes using @daddia/create-space.
---

Use `@daddia/create-space` to produce a complete delivery workspace with the standard directory layout, agent instruction files, IDE configuration, and skill installation -- all from a single command.
...
- An npm token with access to `@daddia/*` packages set in `~/.npmrc`:
...
```bash
pnpm dlx @daddia/create-space my-project --yes --key ACME --use-pnpm
```
...
This installs `@daddia/skills` and runs `sync-skills` as a postinstall step, which copies the skill library into `skills/`. The skills directory is then symlinked into `.cursor/skills` and `.claude/skills` for agent access.
