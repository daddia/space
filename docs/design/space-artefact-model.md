...
- Jonathan Daddia's reference space-library (60-activity prompt library)
...
- The three-package monorepo structure (`@daddia/skills`, `@daddia/create-space`,
  `@daddia/space`)
- The Markdown-only nature of `@daddia/skills`
...
`@daddia/skills` remains the sole publishable skills package. No role-based
sub-packages. Rationale: the LLM router pre-loads all descriptions
regardless of folder; splitting into `@daddia/skills-pm`,
`@daddia/skills-architect`, etc. would multiply changelog, CI, and eval
...
1. Ship the P0 skill changes to `@daddia/skills` in a single minor release
   (0.3.0). Publish; no workspace needs to re-sync yet.
2. Storefront-space pins `@daddia/skills@~0.2.0` until it explicitly opts in.

#### Phase B -- opt-in storefront-space

1. Bump `storefront-space` to `@daddia/skills@^0.3.0`.
...
- **Multi-package split** of `@daddia/skills` by role. Rejected in Section
  3.1.
