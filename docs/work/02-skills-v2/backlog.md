...
**Dependencies.** `@tpw/skills@0.2.x` baseline is stable (SPACE-01 closed). The cart exemplar in `storefront-space` (`domain/cart/`, `work/cart/01-foundations/`, `work/cart/02-add-to-cart/`) is the primary template-quality reference.

**Downstream consumers.** SPACE-03 consumes the `produces`/`consumes` frontmatter fields this WP introduces to build the `space-index` router. Consuming workspaces pick up v2 skills on `pnpm update @tpw/skills`.
...
The epic (SPACE-02) is done when every story is done **and** `@tpw/skills@0.3.0` is published and the human validation session in `storefront-space` passes.
...
- `@tpw/skills@0.3.0` is published with two new skills and three refactored skills.
