...
`@daddia/skills` package structure, `SKILL.md` frontmatter schema, `sync-skills`
bin, and `@daddia/space` source-sync commands are stable and not changed here.
...
- **Profile YAMLs** (new): `packages/skills/profiles/minimal.yaml`,
  ...
  names per profile.
- **`space sync skills`** (new): new `@daddia/space` subcommand that materialises
...
- **Release** `@daddia/skills@0.4.0` with the four new skills and supporting
  generators.
...
`@daddia/create-space`, the `sync-skills` postinstall bin, or the existing
`@daddia/space` sync commands.
...
- §4.2 Level 2 — `@daddia/skills` skill directory layout the generators walk.
- §4.4 Level 2 — `@daddia/space` CLI structure; the new `sync skills` subcommand
...
2. Reads the profile YAML from `node_modules/@daddia/skills/profiles/{name}.yaml`.
...
developer runs: space sync skills --profile domain-team
  ...
  -> resolveProfile("domain-team")
       reads node_modules/@daddia/skills/profiles/domain-team.yaml
  ...
  -> execs sync-skills (postinstall bin)
       ...
       for each skill in @daddia/skills/:
...
or published as a new `@daddia/skills` version consumed by `storefront-space`.
...
| Condition | Behaviour |
| --- | --- |
| Profile name not found in `profiles/` | Exit non-zero: "Profile '{name}' not found in @daddia/skills/profiles/" |
| `.space/profile.yaml` present but malformed YAML | Exit non-zero listing parse error; no skills are synced |
| Skill name in profile YAML does not match any directory in `@daddia/skills` | Warn to stderr: "Skill '{name}' listed in profile not found in source; skipped" |
...
| `@daddia/skills` not installed | Exit non-zero: same message as existing `sync-skills` path |
...
10. `@daddia/skills@0.4.0` is published to the npm registry.
...
- Role views and profiles are shipped with `@daddia/skills@0.4.0`.
