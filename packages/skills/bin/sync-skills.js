 ...
 * sync-skills
 *
 * Run automatically on `pnpm add -D @daddia/skills` (via the package's own
 ...
 *
 * What it does:
 *   1. Copies skills from @daddia/skills (and any extra configured sources) into
 ...
 *
 * Usage:
 *   automatic: fires as postinstall of @daddia/skills and of the consumer repo
...
const sources = [{ name: '@daddia/skills', dir: thisPackageDir }];

for (const sourceName of consumerConfig.sources ?? []) {
  if (sourceName === '@daddia/skills') continue; // already added
  sources.push({
    name: sourceName,
