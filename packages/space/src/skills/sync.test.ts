...
async function makeSkillsPackage(
  ...
  const skillsDir = path.join(workspaceRoot, 'node_modules', '@tpw', 'skills');
  await mkdir(skillsDir, { recursive: true });
  await writeFile(
    path.join(skillsDir, 'package.json'),
    JSON.stringify({ name: '@tpw/skills', version: '0.3.0' }),
  );
...
describe('loadSkillsProfile', () => {
  ...
  it('throws SkillsProfileNotFoundError with the canonical message', async () => {
    ...
    expect(() => loadSkillsProfile(tempDir, 'ghost')).toThrow(
      "Profile 'ghost' not found in @tpw/skills/profiles/",
    );
  });

  it('warns to stderr and skips skills not found in @tpw/skills source', async () => {
