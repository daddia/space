...
describe('detectWorkspaceLayout', () => {
  ...
  beforeEach(async () => {
    tempBase = await mkdtemp(join(tmpdir(), 'ws-layout-test-'));
  });
  ...
  it('returns existingLayout immediately when set to embedded', async () => {
    // Even an empty directory returns 'embedded' when existingLayout is set
  ...
  it('returns existingLayout immediately when set to sibling', async () => {
    // Even a directory with markers returns 'sibling' when existingLayout is set
...
describe('readExistingLayout', () => {
  ...
  beforeEach(async () => {
    tempBase = await mkdtemp(join(tmpdir(), 'ws-layout-read-test-'));
  });
